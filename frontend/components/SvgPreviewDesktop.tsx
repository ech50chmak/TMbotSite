...
export default function SvgPreviewDesktop() {
  ...
  // ===============================
  // Автонастройка viewBox по данным
  // ===============================
  useEffect(() => {
  ...
  return (
    <div
      className="flex w-full h-screen bg-cover bg-center bg-no-repeat bg-[#0f1533] px-[5%] py-4 gap-[40px]"
      style={{ backgroundImage: "url('/background.png')" }}
    >
      {/* Левая панель (параметры) */}
      <div className="w-[320px] flex flex-col gap-[20px] rounded-[40px] bg-[#1b2060]/80 text-[#90d67f] p-6">
        <div className="w-full h-[72px] bg-[#1b2060] rounded-full flex items-center justify-center">
        ...
        <label
          htmlFor="dxfInput"
          className="cursor-pointer w-full bg-[#d6e53c] text-[#1b2060] font-semibold rounded-full px-6 py-3 text-center"
        >
          {dxfName ? `Файл: ${dxfName}` : "Выберите DXF"}
        </label>

        <div className="w-full text-center">
          <label className="w-full block mb-1">Размер плитки (мм)</label>
        ...
        <button
          ...
          disabled={loading}
          className="w-full bg-[#d6e53c] text-[#1b2060] font-semibold rounded-full px-6 py-3"
        >
      ...
      <div className="flex-1 flex flex-col rounded-[40px] bg-[#1b2060]/80 p-4">
        <div className="w-full h-[70px] bg-[#1b2060] rounded-full flex items-center justify-center mb-4">
          <h2 className="text-xl font-bold text-[#90d67f]">{projectName}</h2>
        </div>
        ...
        <div className="flex-1 bg-white rounded-xl overflow-hidden">
          {polygons.length > 0 ? (
            ...
            <p className="text-center mt-6 text-[#1b2060]">
